...
object DAUHandler {
  ...
  def filterDataByRedis(ssc: StreamingContext, startUpLogDStream: DStream[StartUpLog]): DStream[StartUpLog] = {
    ...
    startUpLogDStream.transform((rdd: RDD[StartUpLog]) => {
      ...
      //去重
      val filterStartUpLogRDD: RDD[StartUpLog] = rdd.filter((log: StartUpLog) => {
  ...
  def saveUserToRedis(startUpLogDStream: DStream[StartUpLog]): Unit = {
    ...
    startUpLogDStream.foreachRDD((log: RDD[StartUpLog]) => {

      log.foreachPartition((items: Iterator[StartUpLog]) => {

        //获取Redis连接
        val jedis: Jedis = RedisUtil.getJedisClient

        //遍历写出
        items.foreach((startLog: StartUpLog) => {
